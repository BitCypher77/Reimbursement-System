...
// If profile is not complete, redirect to profile page with message
if (!$profile_complete) {
    $_SESSION['flash_message'] = [
        'type' => 'warning',
        'message' => 'Please complete your profile before submitting a claim. You need to add your Employee ID and Department.'
    ];
...
try {
    ...
    // Check if policies table exists before querying
    $tableCheck = $pdo->query("SELECT name FROM sqlite_master WHERE type='table' AND name='policies'");
    ...
    // Check if projects and clients tables exist before querying
    $tableCheck = $pdo->query("SELECT name FROM sqlite_master WHERE type='table' AND name='projects'");
    $tableCheck2 = $pdo->query("SELECT name FROM sqlite_master WHERE type='table' AND name='clients'");
    if ($tableCheck->fetchColumn() && $tableCheck2->fetchColumn()) {
    ...
    $tableCheck = $pdo->query("SELECT name FROM sqlite_master WHERE type='table' AND name='currencies'");
    if ($tableCheck->fetchColumn()) {
        // Get currencies
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Check that at least one payment proof is provided
        $has_mpesa_code = !empty($mpesa_code);
        $has_receipt = isset($_FILES['receipt']) && $_FILES['receipt']['error'] == 0;
        
        if (!$has_mpesa_code && !$has_receipt) {
            throw new Exception("Please provide either a receipt or an M-PESA transaction code as proof of payment.");
        }
        ...
        $stmt = $pdo->prepare("INSERT INTO claims (
            ...
            description, purpose, incurred_date, receipt_path, status,
            billable_to_client, project_id, submission_date, mpesa_code
...
<div class="px-4 sm:px-6 lg:px-8 py-8">
    <div class="max-w-3xl mx-auto">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
            ...
            <?php if (isset($draftClaim) && $draftClaim !== false): ?>
        ...
        <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="policy_id" id="policy_id" class="mt-1 block w-full py-3 px-4 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                ...
                <!-- Payment Proof Required Message -->
                <div class="mb-4 rounded-md bg-blue-50 dark:bg-blue-900/30 p-4 payment-proof-message hidden">
                    <div class="flex">
                        <div class="flex-shrink-0">
                            <i data-lucide="info" class="h-5 w-5 text-blue-400 dark:text-blue-500"></i>
                        </div>
                        <div class="ml-3">
                            <p class="text-sm text-blue-700 dark:text-blue-200">
                                You must provide <strong>at least one</strong> proof of payment: either upload a receipt or enter an M-PESA transaction code.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <label for="mpesa_code" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                        M-PESA Transaction Code <span class="text-blue-600 font-normal">(or receipt below)</span>
                    </label>
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <i data-lucide="credit-card" class="h-5 w-5 text-gray-400"></i>
                        </div>
                        <input type="text" name="mpesa_code" id="mpesa_code" placeholder="e.g. QWE1234XYZ"
                               class="block w-full pl-10 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                    </div>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Enter the M-PESA transaction code if you paid using mobile money
                    </p>
                </div>
                ...
                <div>
                    <label for="receipt" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                        Receipt <span class="text-blue-600 font-normal">(or M-PESA code above)</span>
                    </label>
                ...
                <div>
                    <div class="flex items-center mb-2">
                        <input type="checkbox" name="is_billable" id="is_billable" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                        <label for="is_billable" class="ml-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Billable to Client/Project
                        </label>
                    </div>
                    
                    <div id="project_selector" class="hidden mt-2">
                        <select name="project_id" id="project_id" class="block w-full py-3 px-4 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    categorySelect.addEventListener('change', function() {
        ...
        if (maxAmount && maxAmount > 0) {
            ...
            maxAmountValue.textContent = `KSH ${parseFloat(maxAmount).toFixed(2)}`;
        } else {
            maxAmountInfo.classList.add('hidden');
        }
        
        // Update payment proof requirements message
        document.querySelector('.payment-proof-message').classList.remove('hidden');
        
        // Toggle receipt required
        receiptRequired.forEach(el => {
            if (isReceiptRequired) {
                el.classList.remove('hidden');
                // Don't set required here - we'll check in form validation
            } else {
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Check if at least one payment proof is provided (either receipt or M-PESA code)
        const hasReceipt = receiptInput.files.length > 0;
        const hasMpesaCode = mpesaInput.value.trim() !== '';
        
        if (!hasReceipt && !hasMpesaCode) {
            e.preventDefault();
            alert('Please provide either a receipt or an M-PESA transaction code as proof of payment.');
            return;
        }
        ...
        if (maxAmount && amount > maxAmount) {
            if (!confirm(`The amount (KSH ${amount.toFixed(2)}) exceeds the maximum allowed (KSH ${maxAmount.toFixed(2)}) for this category. Additional approval may be required. Continue?`)) {
                e.preventDefault();
                return;
            }
        }
        
        if (isBillable.checked && document.getElementById('project_id').value === '') {
            e.preventDefault();
